...
if [ $((maxDepth)) -gt 0 ]; then
	searchString=`find "$searchdir" -maxdepth $maxDepth -print0 | xargs -0 stat -f '%N'`
elif [ "$recursive" = "true" ]; then
	searchString=`find "$searchdir" -print0 | xargs -0 stat -f "%N"`
else
	searchString=`find "$searchdir" -maxdepth 1 -print0 | xargs -0 stat -f "%N"`
fi
		...
		if [ ! -d $file ]; then
			if [ -r $file ]; then 
				...
				if [ "$tempResultA" ]; then 
					...
					if [ "`which exiv2`" ]; then
						output=`exiv2 -u pr "$file"  2>&1 | grep -n -e "$pattern"`
					else
						continue
					fi
				else
					output=`cat "$file" | grep -n -e $pattern`
				fi
